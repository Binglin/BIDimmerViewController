...
//        self.backgroundColor = [UIColor lightGrayColor];
        ...
        self.imageButton.frame = CGRectMake(0, 0, CGRectGetWidth(frame), CGRectGetWidth(frame));
...
- (void)loadView{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:({ CGRect frame = CGRectZero;
        ...
        if (self.actions.count <= gridLayout.columnCount) {
            CGFloat horizontalSpace = ([UIScreen mainScreen].bounds.size.width - self.actions.count * gridLayout.itemSize.width)/(self.actions.count + 1);
            CGFloat verticalSpace  = gridLayout.minimumLineSpacing;
